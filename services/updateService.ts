...
const RELEASE_API = `https://api.github.com/repos/${REPO_SLUG}/releases`;
const RELEASE_PAGE = `https://github.com/${REPO_SLUG}/releases`;
const REPO_PAGE = `https://github.com/${REPO_SLUG}`;
...
async function fetchLatestRelease(headers: Record<string, string>) {
  // Fetch lists of releases, taking the first one (newest)
  const resp = await fetch(`${RELEASE_API}?per_page=1`, { headers });
  if (!resp.ok) {
